...
function gameIO() {
  ...
  game.touch = function() {
    ...
    window.addEventListener( "touchmove", function( event ) {
      ...
      for( var i = 0; i < event.targetTouches.length; i++ ) {
		...
		console.log("person is touching at"+touches[i].x+" , "+touches[i].y);
      }
    } );
  ...
  game.image = function( image, x, y, width, height, opacity ) {
    ...
    element.type = "image";
	element.disablesmooth = false;
    element.renderSpecific = function( ctx, ratio ) {
		if(element.disablesmooth){
  ...
  game.spritesheetimage = function( image, x, y, width, height, opacity,frameIndex,numberOfFrames) {
    var element = new game.object();
    element.image = image || null;
    element.imagestats = [element.image.width,element.image.height]
    element.position = new game.Vector2( x || 0, y || 0 );
    element.width = width || 100;
    element.height = height || 100;
    element.opacity = opacity || 1;
    ...
    element.numberOfFrames = numberOfFrames;
   // element.d = [dx,dy,dw,dh];
    ...
    element.renderSpecific = function( ctx, ratio ) {
		...
		ctx.drawImage(
           ...
           this.width / this.numberOfFrames,
           this.height);
     // ctx.drawImage( this.image, -this.width / 2 / ratio, -this.height / 2 / ratio, this.width / ratio, this.height / ratio,this.d[0],this.d[1],this.d[2],this.d[3] );
	  if(element.disablesmooth){
			ctx.imageSmoothingEnabled = true;
		}
    }
  ...
  game.animatedImage = function( images, x, y, width, height, opacity,repeat,pertick ) {
    var element = new game.object();
    element.images = images || null;
	element.onimage = 0;
    element.position = new game.Vector2( x || 0, y || 0 );
    element.width = width || 100;
    element.height = height || 100;
    element.opacity = opacity || 1;
	element.repeat = repeat||false;
    element.type = "image";
	element.pertick = pertick||1;
	element.disablesmooth = false;
    element.renderSpecific = function( ctx, ratio ) {
      ...
      ctx.drawImage( this.images[Math.round(element.onimage)], -this.width / 2 / ratio, -this.height / 2 / ratio, this.width / ratio, this.height / ratio );
	  if(element.disablesmooth){
			ctx.imageSmoothingEnabled = true;
		}
		if(element.onimage >= element.images.length-1){
			if(!element.repeat){
		...
		element.onimage+=element.pertick;
    }
    return element;
  }
  game.animatedImadge = function( images, x, y, width, height, opacity,repeat,pertick ) {
    var element = new game.object();
    element.images = images || null;
	  element.onimage = 0;
    element.position = new game.Vector2( x || 0, y || 0 );
    ...
    element.renderSpecific = function( ctx, ratio ) {
      ...
      ctx.drawImage( this.images, -this.width / 2 / ratio, -this.height / 2 / ratio, this.width / ratio, this.height / ratio );
	  if(element.disablesmooth){
			ctx.imageSmoothingEnabled = true;
		}
		if(element.onimage >= element.images.length-1){
    ...
    function down(e) {
      ...
      if (e.keyCode == 37 || e.keyCode == 65) {
        ...
        if (!control.enter) {
          changed = true;
          control.enter = true;
					control.changedKeys.push( "enter" );
        }
      }
  ...
  game.arc = function( x, y, radius, color, angle, rotation, lineWidth, opacity ) {
    var element = new game.object();
    element.position = new game.Vector2( x || 0, y || 0 );
    element.radius = radius || 100;
    element.color = color || "#000000";
  ...
  game.notification = function( text, x, y, fillStyle, font, fontSize, duration, otherParams, opacity ) {
    var element = new game.object();
    element.text = text || "";
    element.position = new game.Vector2( x || 0, y || 0 );
    element.fillStyle = fillStyle || "#000";
  ...
  game.imagenotification = function( image, x, y, width, height, opacity, duration ) {
    var element = new game.object();
    element.image = image || null;
    element.position = new game.Vector2( x || 0, y || 0 );
    element.width = width || 100;
  ...
  game.packetFunctions = {
	...
	"getplayers" : function( packet ) {
			console.log("players on"+packet);
    },
		// Add
    "x" : function( packet ) {
